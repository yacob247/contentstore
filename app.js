            ...
            container.innerHTML = state.draftFiles.map(df => `
                <div class="bg-gray-900 border border-gray-700 p-5 rounded-xl relative group hover:border-gray-500 transition-colors shadow-sm">
                    ...
                    <div class="bg-gray-800/80 p-4 rounded-lg border border-gray-700">
                        ...
                        ${df.inputType === 'url' ? `<div class="relative"><i class="fa-solid fa-globe absolute left-3 top-3 text-gray-500"></i><input type="url" value="${df.url}" onchange="updateDraft(${df.id}, 'url', this.value)" required class="w-full bg-gray-900 border border-gray-600 rounded-lg pl-9 pr-3 py-2 text-white text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500" placeholder="https://..."></div>` : `<input type="file" onchange="handleFile(${df.id}, this)" required class="w-full text-sm text-gray-300 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-bold file:bg-blue-600 file:text-white hover:file:bg-blue-700 file:transition-colors file:cursor-pointer cursor-pointer">`}
                    </div>
                </div>
            `).join('');
        }
